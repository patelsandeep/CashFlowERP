...
public interface CostRateSourceTranslationService {

    @Nonnull
    @Info("The translated label for cost rate Source")
    @DynamicDefault(CostRateSourceDefaultTranslator.class)
    String getCostRateSourceLabel(
            @ConfigParam(name = "costRateSource") @Nonnull final CostRateSource costRateSource);

    static final class CostRateSourceDefaultTranslator implements DefaultConfigurator {
        ...
        public String getDefault(final Method configMethod, final Object[] args) {
            ...
            return ((CostRateSource) args[0]).toString();
        }
